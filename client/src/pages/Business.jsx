...
const Business = () => {
  ...
  useEffect(() => {
    const fetchBusinesses = async () => {
      try {
        ...
        setFilteredBusinesses(dataArray);
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (filter === "all") {
      setFilteredBusinesses(safeBusinesses);
    } else {
      setFilteredBusinesses(
        safeBusinesses.filter((biz) => biz?.industry?.toLowerCase() === filter.toLowerCase())
      );
    }
  }, [filter, businesses]);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <header className="mb-10 flex flex-col justify-between gap-6 md:flex-row md:items-end">
        <div className="max-w-2xl">
          ...
          <p className="mt-3 text-base text-slate-500">
            Connect directly with verified regional vendors, traditional artisans, accommodations, and guided travel operators.
          </p>
        </div>
      ...
      <nav className="mb-8 flex flex-wrap gap-2.5" aria-label="Business Industries">
        {["all", "stay", "artisan", "guide", "food"].map((cat) => (
          <button
            key={cat}
      ...
      <main className="rounded-3xl bg-slate-50 border border-slate-100 p-6 sm:p-10">
        {loading && (
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3].map((item) => (
              <div key={item} className="h-44 w-full animate-pulse rounded-2xl bg-slate-200" />
            ))}
          </div>
        )}
        ...
        {!loading && !error && filteredBusinesses.length === 0 && (
          <div className="py-16 text-center">
            ...
            <p className="mt-1 text-sm text-slate-500">Try choosing another category filter.</p>
          </div>
        )}
      </main>
    </div>
